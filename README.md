...
data <- subset(data, select =-c(3,4,5,6,9,10)) # Removes upper and lower values 
...
missmap(data, col=c("red3", "#1380A1"),legend = TRUE) # Checks for missing data by producing a map (red points indicts missing data). 
...
#### Bar graphs : SDI analysis low and high ################# 
...
mean(sdi_2010_2019$`low-SDI`)
mean(sdi_2010_2019$`high-SDI`)
